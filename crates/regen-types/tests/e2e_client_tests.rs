use regen_types::{client::{Client, ClientConfigBuilder}, types};
...
async fn test_mainnet_with_test_signer() {
    ...
    let client = Client::new(config, Some(signer)).await
...
async fn test_query_all_credit_classes() {
    ...
    let mut ecocredit_client = types::regen::ecocredit::v1::query_client::QueryClient::new(client.channel.clone());
    ...
    let response = ecocredit_client.classes(request).await
    ...
    assert!(!classes.is_empty(), "Expected to find at least one credit class");
    ...
    for class in &classes {
        ...
        assert!(!class.credit_type_abbrev.is_empty(), "Credit type should not be empty");
    }
...
async fn test_query_credit_classes_with_pagination() {
    ...
    let request = types::regen::ecocredit::v1::QueryClassesRequest {
        pagination: Some(cosmos_sdk_proto::cosmos::base::query::v1beta1::PageRequest {
            key: vec![],
            offset: 0,
            limit: 5,
            count_total: true,
    ...
    assert!(classes.len() <= 5, "Should get at most 5 classes due to pagination");
